;----------------------------------------------------------------------------------------
function fourier_highpass_ideal, freq_dist, cut_freq, double = double, greater = greater
;----------------------------------------------------------------------------------------
...
; ***  lower     = (1/0) if set use gt rather than ge to determine cut.
;--(output)------------------------------------------------------------------------------
...
;-----------------------------------------------------------------------------------------
  ...
  if keyword_set(equal) then begin
    if keyword_set(double) then taper_vals = double(freq_dist ge cut_freq) else taper_vals = float(freq_dist ge cut_freq)
  endif else if keyword_set(double) then taper_vals = double(freq_dist gt  cut_freq) else taper_vals = float(freq_dist gt cut_freq)
