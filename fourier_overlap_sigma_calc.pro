;----------------------------------------------------------------------------------------
function fourier_overlap_sigma_calc, x, a, b, c, d, covariance_matrix
;----------------------------------------------------------------------------------------
;
;--(input)-------------------------------------------------------------------------------
...
;-----------------------------------------------------------------------------------------
  ...
  if n_elements(x) gt 1 then f_error,['fourier_overlap_sigma_calc: the input variable must have only one element']
  ...
  sigma_vec = [sigma_a, sigma_b, sigma_c, sigma_d]
 ...
 ; partial derivatives of a symmetric sigmoidal for each parameter
  ...
  differentials_vec = [aderiv, bderiv, cderiv, dderiv]
  ...
  for ii = 0, n_variables - 1, 1 do begin
    for jj = 0, n_variables - 1, 1 do begin
      if ii ne jj then continue
      coorelation_coefficient = covariance_matrix[ii,jj]/(sigma_vec[ii] * sigma_vec[jj])
      sigma_parts_arr[ii,jj] = differentials_vec[ii] * differentials_vec[jj] * coorelation_coefficient * sigma_vec[ii] * sigma_vec[jj]

      ; sigma_parts_arr[ii,jj] = differentials_vec[ii] * differentials_vec[jj] * covariance_matrix[ii,jj] * sigma_vec[ii] * sigma_vec[jj]
