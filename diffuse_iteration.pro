pro diffuse_iteration, master_inputfile
  ...
  while(iter_break eq 0 && iter_num lt iter_nmax) do begin
    ...
    ; write out the lowpass filtered star image
    star_smoothfile = strcompress("starfile_smooth" + string(iter_num) + ".fits",/remove_all) ; change starfile for input_file
    star_smoothpath = strcompress(datadir + star_smoothfile, /remove_all)
    fourier_filter_tool, filter_choice, iter_bwo, 'low', star_cut_length, $ ; description of the filter
      base_starfile, $ ; input image by filename
      filtered_image_path = star_smoothpath ; output file for the filtered image
    ...
    ; write out the lowpass filtered gas image
    gas_smoothfile = strcompress("gasfile_smooth" + string(iter_num) + ".fits",/remove_all) ; change gasfile for input_file
    gas_smoothpath = strcompress(datadir + gas_smoothfile, /remove_all)
    fourier_filter_tool, filter_choice, iter_bwo, 'low', gas_cut_length, $ ; description of the filter
      base_gasfile, $ ; input image by filename
      filtered_image_path = gas_smoothpath ; output file for the filtered image
