...
;----------------------------------------------------------------------------------------
  ...
  if n_elements(peakiddir) ne 1 then peakiddir = '-'
  if n_elements(starpeakidfile) ne 1 then starpeakidfile = '-'
  if n_elements(gaspeakidfile) ne 1 then gaspeakidfile = '-'
  if n_elements(intpeakidfile) ne 1 then intpeakidfile = '-'
  ...
  if iter_input_switch eq 1 then begin ; create input file for iteration
    ...
    printf, inp_lun, 'use_guess       ', use_guess_str,      '#  [0] Do not Filter images before the first Heisenberg run and ignore parameter: "initial_guess" [1] Filter images with initial guess of lambda before the first Heisenberg run'
    ...
    printf, inp_lun, 'iter_crit_len   ', iter_crit_len_str,  '#  Number of previous runs of Heisenberg over which iter_criterion must be true, i.e. if iter_crit_len = 1 then the current value of lambda is compared to the previous value of lambda, if iter_crit_len = 2 then then the current value of lambda is compared to the previous value of lambda and the value of lambda prior to the previous value'
    printf, inp_lun, 'iter_nmax       ', iter_nmax_str,      '#  Maximum number of Heisenberg runs allowed in the iteration process. If iter_criterion is not reached before this number of runs the programme will exit anyway.'
    ...
    printf, inp_lun, 'use_nice        ', use_nice_str,       '# (on/off) Use the "nice" command when spawning Heisenberg runs to the terminal to adjust the process priority. Note that this may not work depending on the system configuration'
